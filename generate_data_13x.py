...
root_dir = '/vpalab/disk_dataset/leo/datasets/citypersons'
...
for type in types:
    ...
    res_path = os.path.join('../data/citypersons', type)
    image_data = []
    ...
    for l in range(len(annos[index][0])):
        ...
        gts = anno[0][0][2]
        gts[:, 1:] = gts[:, 1:] * 1328.0 / 1024.0

        img_path = os.path.join(all_img_path, type + '/' + imgname)
        # im = cv2.imread(img_path)
        # im_resize = cv2.resize(im, (2656, 1328))
        img_resize_path = os.path.join(root_dir, 'images_13x', type + '/' + imgname)
        # cv2.imwrite(img_resize_path, im_resize)
        ...
        annotation = {}
        annotation['filepath'] = img_resize_path
